...
use App\Models\ScenarioEight;
use App\Models\EmployeeDetail;
use App\Models\LogXMLGenActivity;
use App\Models\DirectorIdDetail;
use App\Models\SignatoryDetail;

class ScenarioEightAllController extends Controller
{
    //
    function __construct()
    {
        $this->middleware('permission:scenario-eight-all-list|scenario-eight-all-edit|scenario-eight-all-delete|scenario-eight-delete', ['only' => ['list']]);
        $this->middleware('permission:scenario-eight-all-edit', ['only' => ['edit', 'update']]);
        $this->middleware('permission:scenario-eight-all-delete', ['only' => ['destroy']]);
    }

    public function list(Request $request)
    {
        if ($request->ajax()) {
            $data = LogXMLGenActivity::select('*')->where('scenario_no', 8)->where('is_delete', 0)->orderBy('created_at', 'desc')->get();
            //var_dump($data); exit();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('excel', function ($row) {
                    $download_url = url('download-excel-eight/' . encrypt($row->id));
                    return '<a href="' . $download_url . '">Download Excel</a>';
                })
                ->addColumn('edit', function ($row) {
                    $edit_url = url('/scenario-eight-edit/' . encrypt($row->id));
                    $btn = '<a href="' . $edit_url . '"><i class="fa fa-edit"></i></a>';
                    return $btn;
                })
                ->addColumn('blockscenarioeightxml', 'adminpanel.generate_xml.scenario_eight.all.actionsBlockXml')
                ->addColumn('xmlsubmitted', function ($row) {
                    if ($row->status == "Y") {
                        $status = 'fa fa-check';
                        $btn = '<a href="changestatus-scenario-eight-all-xml/' . $row->id . '/' . $row->cEnable . '"><i class="' . $status . '"></i></a>';
                    } else {
                        $status = 'fa fa-remove';
                        $btn = 'Submitted';
                    }
                ...
                ->rawColumns(['excel', 'edit', 'blockscenarioeightxml', 'xmlsubmitted'])
                ->make(true);
        }

        return view('adminpanel.generate_xml.scenario_eight.all.list');
    }
    ...
    public function edit($id)
    {
        try {
            ...
            $info = ScenarioEight::findOrFail($ID); // Using findOrFail to throw an exception if not found
            $directors = DirectorIdDetail::where('entity_id', $ID)->where('scenario_no', 8)->get();
            $signatories = SignatoryDetail::where('entity_id', $ID)->where('scenario_no', 8)->get();

            return view('adminpanel.generate_xml.scenario_eight.all.edit', [
                'data' => $info,
                'directors' => $directors,
    ...
    public function blockXML(Request $request)
    {
        ...
        ScenarioEight::where('xml_gen_status', 'Y')
        ...
        return redirect()->route('scenario-eight-all-list')
    ...
    {
        $ID = decrypt($id);
        $log_data = LogXMLGenActivity::select('*')->where('id', $ID)->where('scenario_no', 8)->first();
        $from_date = $log_data->from_date;
        $to_date = $log_data->to_date;
        $xml_type = $log_data->xml_type;

        if ($request->ajax()) {
            $data = ScenarioEight::select(
                'scenario_8_trans_details.id as ent_id',
                'scenario_8_trans_details.status as trans_status',
                'scenario_8_trans_details.*'
            )
                ->distinct()  // Use distinct to avoid duplicates
                ->leftJoin('signatory_details', 'scenario_8_trans_details.id', '=', 'signatory_details.entity_id')
                ->leftJoin('director_details', 'scenario_8_trans_details.id', '=', 'director_details.entity_id')
                ->where('scenario_8_trans_details.is_delete', 0)
                ->where('scenario_8_trans_details.xml_gen_status', 'N')
                ->whereBetween('scenario_8_trans_details.date_transaction', [$log_data->from_date, $log_data->to_date])
                ->orderBy('scenario_8_trans_details.id', 'asc')
                ->get();
            //var_dump($data); exit();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('edit', function ($row) {
                    $edit_url = url('/edit-scenario-eight-all/' . encrypt($row->ent_id));
                    $btn = '<a href="' . $edit_url . '"><i class="fa fa-edit"></i></a>';
                    return $btn;
                })
                ->addColumn('activation', function ($row) {
                    if ($row->trans_status == "Y")
                        ...
                        $status = 'fa fa-remove';
                    $btn = '<a href="changestatus-scenario-eight-all/' . $row->ent_id . '/' . $row->cEnable . '"><i class="' . $status . '"></i></a>';
                    return $btn;
                })
                ->addColumn('blockscenarioeight', 'adminpanel.generate_xml.scenario_eight.all.actionsBlock')
                ->rawColumns(['edit', 'activation', 'blockscenarioeight'])
                ->make(true);
        }

        return view('adminpanel.generate_xml.scenario_eight.all.datalist', ['log_id' => $id, 'from_date' => $from_date, 'to_date' => $to_date, 'xml_type' => $xml_type]);
    }
    ...
    public function update(Request $request)
    {
        ...
        return redirect()->route('scenario-eight-all-list')->with('success', 'Record updated successfully.');
    }


    public function activation(Request $request)
    {
        $data =  ScenarioEight::find($request->id);
    ...
    public function generate_xml()
    {
        ...
        // $data = ScenarioEight::whereBetween('created_at', [$from_date, $to_date])->where('xml_gen_status', '=', 'N')->where('scenario_type',$scenario_type)
        //   ->groupBy('rentity_id')
        // ->first();

        $data = ScenarioEight::select('rentity_id', 'rentity_branch', 'submission_code', 'report_code', 'entity_reference', 'submission_date', 'currency_code_local')
            ->whereBetween('date_transaction', [$from_date, $to_date])
        ...
        if ($scenario_type == 'Entity') {
            $xml_type = 'Entity';
            $trans = ScenarioEight::whereBetween('date_transaction', [$from_date, $to_date])
        ...
        // dd($scenario_type);
        // \DB::enableQueryLog();
        $indicators = ScenarioEight::select('report_indicator')
            ->whereBetween('date_transaction', [$from_date, $to_date])
        ...
        $fileName = 'files/xmlfile_' . $formatted_from_date . '_' . $formatted_to_date . '_' . time() . '_scenario_eight.xml';
        $xmlDoc->save(storage_path('app/public/' . $fileName));
        $scenario_no = 8;

        // Log the creation of the XML
        \Log::info('User ' . $user->id . ' created an XML file for Scenario 8 at ' . Carbon::now());
        \LogActivity::addToLogXMLGen('User ' . $user->id . ' created an XML file for Scenario 8 at ' . Carbon::now(), $from_date, $to_date, $fileName, $xml_type, $scenario_no, $xml_gen_status);
    ...
    public function downloadExcel($id)
    {
        ...
        $excel_data = ScenarioEight::select('*')
            ->where('is_delete', '0')
            ->where('status', '=', 'Y')
        ...
        $filename = Carbon::now() . '-scenario-eight.csv';
        ...
        foreach ($excel_data as $row) {
            $num_from_directors = DirectorIdDetail::where('entity_id', $row->id)->where('scenario_no',8)->where('entity_type','From')->count();
            $num_from_signatories = SignatoryDetail::where('entity_id', $row->id)->where('scenario_no',8)->where('entity_type','From')->count();
            ...
            $num_to_directors = DirectorIdDetail::where('entity_id', $row->id)->where('scenario_no',8)->where('entity_type','To')->count();
            $num_to_signatories = SignatoryDetail::where('entity_id', $row->id)->where('scenario_no',8)->where('entity_type','To')->count();
        ...
        $callback = function () use ($excel_data, $columns, $max_from_directors, $max_to_directors, $max_from_signatories , $max_to_signatories) {
            ...
            foreach ($excel_data as $row) {
                ...
                $directors_from = DirectorIdDetail::where('entity_id', $row->id)->where('scenario_no',8)->where('entity_type','From')->get();
                ...
                $directors_to = DirectorIdDetail::where('entity_id', $row->id)->where('scenario_no',8)->where('entity_type','To')->get();
                ...
                $signatories_from = SignatoryDetail::where('entity_id', $row->id)->where('scenario_no',8)->where('entity_type','From')->get();
                ...
                $signatories_to = SignatoryDetail::where('entity_id', $row->id)->where('scenario_no',8)->where('entity_type','To')->get();
