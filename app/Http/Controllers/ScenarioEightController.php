...
class ScenarioEightController extends Controller
{
    public function list(Request $request)
    {

        if ($request->ajax()) {
            $data = ScenarioEight::select('*')->where('is_delete', '0')->where('status', 'Y')->where('xml_gen_status', 'N')->orderBy('id', 'DESC')->get();
            //var_dump($data); exit();
        ...
        return view('adminpanel.generate_xml.scenario_eight.list');
    }
    ...
    public function fetchLastDetail(Request $request)
    {
        ...
        $lastDetail = LogXMLGenActivity::where('xml_type', $scenarioType)->where('xml_gen_status',1)->where('scenario_no',8)
            ->orderBy('id', 'desc')
            ->first();
    ...
    public function generate_xml()
    {
        ...
        // dd($scenario_type , $from_date , $to_date);
        ...
        // ScenarioEight::whereBetween('date_transaction', [$from_date, $to_date])
