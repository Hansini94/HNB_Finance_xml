...
class ScenarioOneAllController extends Controller
{
    ...
    {
        $this->middleware('permission:scenario-one-all-list|scenario-one-all-edit|scenario-one-all-delete', ['only' => ['list']]);
    ...
    {
        if ($request->ajax()) {
            $data = ScenarioOne::select('*')->where('is_delete', '0')->where('xml_gen_status', 'Y')->orderBy('id', 'asc')->get();
            //var_dump($data); exit();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('edit', function ($row) {
                    $edit_url = url('/edit-scenario-one-all/' . encrypt($row->id));
                 ...
                 ->addColumn('activation', function($row){
                     if ( $row->status == "Y" )
                         ...
                         $status ='fa fa-remove';
                     $btn = '<a href="changestatus-scenario-one-all/'.$row->id.'/'.$row->cEnable.'"><i class="'.$status.'"></i></a>';
                     return $btn;
                 })
    ...
    public function edit($id)
    {
        ...
        //
        $ID = decrypt($id);
        $info = ScenarioOne::where('id', '=', $ID)->first();


        return view('adminpanel.generate_xml.scenario_one.all.edit', ['data' => $info]);
    }
    ...
    public function update(Request $request)
    {
        ...
        \LogActivity::addToLog('Scenario One Record updated('.$id.').');
    ...
    public function generate_xml()
    {
        ...
        $xml_type = '';
        $xml_gen_status=2; //new generation 1 , old data generation 2

        // Retrieve data from the database
        $data = ScenarioOne::whereBetween('created_at', [$from_date, $to_date])->where('xml_gen_status', '=', 'Y')->where('account_type',$scenario_type)
        ...
        $fileName = 'old_files/xmlfile_' . time() . '.xml';
        ...
        \LogActivity::addToLogXMLGen('User ' . $user->id . ' created an XML file at ' . Carbon::now(),$from_date,$to_date,$xml_type,$xml_gen_status);
