...
class ScenarioSevenController extends Controller
{
    public function list(Request $request)
    {

        if ($request->ajax()) {
            $data = ScenarioSeven::select('*')->where('is_delete', '0')->where('status', 'Y')->where('xml_gen_status', 'N')->orderBy('id', 'DESC')->get();
            //var_dump($data); exit();
        ...
        return view('adminpanel.generate_xml.scenario_seven.list');
    }
    ...
    public function fetchLastDetail(Request $request)
    {
        ...
        $lastDetail = LogXMLGenActivity::where('xml_type', $scenarioType)->where('xml_gen_status',1)->where('scenario_no',7)
            ->orderBy('id', 'desc')
            ->first();
    ...
    public function generate_xml()
    {
        ...
        // ScenarioSeven::whereBetween('date_transaction', [$from_date, $to_date])
