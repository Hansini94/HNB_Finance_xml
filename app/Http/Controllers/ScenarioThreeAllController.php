...
class ScenarioThreeAllController extends Controller
{
    ...
    {
        $this->middleware('permission:scenario-three-all-list|scenario-three-all-edit|scenario-three-all-delete|scenario-three-delete', ['only' => ['list']]);
    ...
    {
        if ($request->ajax()) {
            $data = LogXMLGenActivity::select('*')->where('scenario_no', 3)->where('is_delete', 0)->orderBy('created_at', 'desc')->get();
            //var_dump($data); exit();
            return Datatables::of($data)
                ...
                ->addColumn('blockscenariothreexml', 'adminpanel.generate_xml.scenario_three.all.actionsBlockXml')
                ->addColumn('xmlsubmitted', function ($row) {
                    if ($row->status == "Y") {
                        $status = 'fa fa-check';
                        $btn = '<a href="changestatus-scenario-three-all-xml/' . $row->id . '/' . $row->cEnable . '"><i class="' . $status . '"></i></a>';
                    } else {
                        $status = 'fa fa-remove';
                        $btn = 'Submitted';
                    }
                ...
                ->rawColumns(['excel', 'edit', 'blockscenariothreexml', 'xmlsubmitted'])
                ->make(true);
        }
    ...
    public function edit($id)
    {
        try {
            ...
            $info = ScenarioThree::findOrFail($ID); // Using findOrFail to throw an exception if not found
            $directors = DirectorIdDetail::where('entity_id', $ID)->where('scenario_no', 3)->get();
            $signatories = SignatoryDetail::where('entity_id', $ID)->where('scenario_no', 3)->get();

            return view('adminpanel.generate_xml.scenario_three.all.edit', [
                'data' => $info,
                'directors' => $directors,
    ...
    public function blockXML(Request $request)
    {
        ...
        ScenarioThree::where('xml_gen_status', 'Y')
            ->whereBetween('date_transaction', [$from_date, $to_date])
            ->update(['xml_gen_status' => 'N']);

        \LogActivity::addToLog('Scenario Three XML Record deleted(' . $id . ').');
    ...
    public function activationXml(Request $request)
    {
        ...
        if ($data->status == "Y") {
            ...
            \LogActivity::addToLog('Scenario Three Record Submitted to GoAML(' . $id . ').');
    ...
    public function datalist(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            $data = ScenarioThree::select(
                'scenario_3_trans_details.id as ent_id',
                'scenario_3_trans_details.status as trans_status',
                'scenario_3_trans_details.*'
            )
                ->distinct()  // Use distinct to avoid duplicates
                ->leftJoin('signatory_details', 'scenario_3_trans_details.id', '=', 'signatory_details.entity_id')
                ->leftJoin('director_details', 'scenario_3_trans_details.id', '=', 'director_details.entity_id')
                ->where('scenario_3_trans_details.is_delete', 0)
                ->where('scenario_3_trans_details.xml_gen_status', 'N')
                ->whereBetween('scenario_3_trans_details.date_transaction', [$log_data->from_date, $log_data->to_date])
                ->orderBy('scenario_3_trans_details.id', 'asc')
                ->get();
            //var_dump($data); exit();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('edit', function ($row) {
                    $edit_url = url('/edit-scenario-three-all/' . encrypt($row->ent_id));
                    $btn = '<a href="' . $edit_url . '"><i class="fa fa-edit"></i></a>';
                    return $btn;
                })
                ->addColumn('activation', function ($row) {
                    if ($row->trans_status == "Y")
                        ...
                        $status = 'fa fa-remove';
                    $btn = '<a href="changestatus-scenario-three-all/' . $row->ent_id . '/' . $row->cEnable . '"><i class="' . $status . '"></i></a>';
                    return $btn;
                })
    ...
    public function update(Request $request)
    {
        ...
        // Log activity
        \LogActivity::addToLog('Scenario Three Record updated (ID: ' . $id . ').');
    ...
    public function generate_xml()
    {
        ...
        $fileName = 'files/xmlfile_' . $formatted_from_date . '_' . $formatted_to_date . '_' . time() . '_scenario_three.xml';
    ...
    public function downloadExcel($id)
    {
        ...
        foreach ($excel_data as $row) {
            $num_directors = DirectorIdDetail::where('entity_id', $row->id)->where('scenario_no',3)->count();
            $num_signatories = SignatoryDetail::where('entity_id', $row->id)->where('scenario_no',3)->count();
        ...
        $callback = function () use ($excel_data, $columns, $max_directors, $max_signatories) {
            ...
            foreach ($excel_data as $row) {
                $rowData = [
                    ...
                    $row->from_account_incorporation_legal_form,
                    $row->from_account_incorporation_number,
                    $row->from_account_business,
                    $row->from_account_address_type,
                    $row->from_account_address,
                    $row->from_account_city,
                    $row->from_account_country_code,
                    $row->from_account_incorporation_country_code,
                    $row->status_code,
                    $row->from_country,
                    $row->to_funds_code,
                ...
                $directors = DirectorIdDetail::where('entity_id', $row->id)->where('scenario_no',3)->get();
                ...
                $signatories = SignatoryDetail::where('entity_id', $row->id)->where('scenario_no',3)->get();
