...
use App\Models\ScenarioThree;
use App\Models\EmployeeDetail;
use App\Models\LogXMLGenActivity;

class ScenarioThreeAllController extends Controller
{
    //
    function __construct()
    {
        $this->middleware('permission:scenario-three-all-list|scenario-three-all-edit|scenario-three-all-delete', ['only' => ['list']]);
        $this->middleware('permission:scenario-three-all-edit', ['only' => ['edit', 'update']]);
        $this->middleware('permission:scenario-three-all-delete', ['only' => ['destroy']]);
    }

    public function list(Request $request)
    {
        if ($request->ajax()) {
            $data = LogXMLGenActivity::select('*')->where('scenario_no',3)->orderBy('created_at', 'desc')->get();
            //var_dump($data); exit();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('excel', function ($row) {
                    $download_url = url('download-excel-three/' . encrypt($row->id));
                    return '<a href="' . $download_url . '">Download Excel</a>';
                })
                ->addColumn('edit', function ($row) {
                    $edit_url = url('/scenario-three-edit/' . encrypt($row->id));
        ...
        return view('adminpanel.generate_xml.scenario_three.all.list');
    }

    public function datalist(Request $request, $id)
    {
        $ID = decrypt($id);
        $log_data = LogXMLGenActivity::select('*')->where('id', $ID)->where('scenario_no',3)->first();
        $from_date = $log_data->from_date;
        $to_date = $log_data->to_date;
        $xml_type = $log_data->xml_type;

        if ($request->ajax()) {
            $data = ScenarioThree::select('*')->where('is_delete', '0')->where('xml_gen_status', 'N')->whereBetween('date_transaction', [$log_data->from_date, $log_data->to_date])->orderBy('id', 'asc')->get(); // xml_gen_status', 'N' => Y
            //var_dump($data); exit();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('edit', function ($row) {
                    $edit_url = url('/edit-scenario-three-all/' . encrypt($row->id));
                    $btn = '<a href="' . $edit_url . '"><i class="fa fa-edit"></i></a>';
                    return $btn;
                 })
                 ->addColumn('activation', function($row){
                     if ( $row->status == "Y" )
                         ...
                         $status ='fa fa-remove';
                     $btn = '<a href="changestatus-scenario-three-all/'.$row->id.'/'.$row->cEnable.'"><i class="'.$status.'"></i></a>';
                     return $btn;
                 })
                 ->addColumn('blockscenariothree', 'adminpanel.generate_xml.scenario_three.all.actionsBlock')
                 ->rawColumns(['edit','activation','blockscenariothree'])
                ->make(true);
        }

        return view('adminpanel.generate_xml.scenario_three.all.datalist', ['log_id' => $id , 'from_date' => $from_date , 'to_date' => $to_date , 'xml_type' => $xml_type]);
    }
    ...
    public function edit($id)
    {
        ...
        //
        $ID = decrypt($id);
        $info = ScenarioThree::where('id', '=', $ID)->first();


        return view('adminpanel.generate_xml.scenario_three.all.edit', ['data' => $info]);
    }
    ...
    public function update(Request $request)
    {
        ...
        $data = ScenarioThree::find($id);
        $data->update($input);

        $id = $data->id;


        \LogActivity::addToLog('Scenario Three Record updated('.$id.').');

        return redirect()->route('scenario-three-all-list')->with('success', 'Record updated successfully.');
    }
    ...
    {
        $data =  ScenarioThree::find($request->id);
        ...
        if ( $data->status == "Y" ) {
            ...
            \LogActivity::addToLog('Scenario Three Record deactivated('.$id.').');

            return redirect()->route('scenario-three-all-list')
            ...
            \LogActivity::addToLog('Scenario Three Record activated('.$id.').');
    ...
    public function block(Request $request)
    {
        ...
        \LogActivity::addToLog('Scenario Three Record deleted('.$id.').');
    ...
    public function generate_xml()
    {
        ...
        // $data = ScenarioThree::whereBetween('created_at', [$from_date, $to_date])->where('xml_gen_status', '=', 'N')->where('scenario_type',$scenario_type)
         //   ->groupBy('rentity_id')
           // ->first();

        $data = ScenarioThree::select('rentity_id', 'rentity_branch', 'submission_code', 'report_code', 'entity_reference', 'submission_date', 'currency_code_local')
        ...
        {
            $xml_type = 'Entity';
            $trans = ScenarioThree::whereBetween('date_transaction', [$from_date, $to_date])
        ...
        else
        {
            ...
            foreach ($trans as $item) {
                ...
                if($item->from_person_signatory_is_primary !== null && $item->from_person_signatory_is_primary !== '')
                {
                    $signatory->appendChild($xmlDoc->createElement('is_primary', $item->from_person_signatory_is_primary));
                }

                $t_person = $xmlDoc->createElement('t_person');

                if($item->from_person_signatory_gender !== null && $item->from_person_signatory_gender !== '')
                {
                    $t_person->appendChild($xmlDoc->createElement('gender', $item->from_person_signatory_gender));
                }
                if($item->from_person_signatory_title !== null && $item->from_person_signatory_title !== '')
                {
                    $t_person->appendChild($xmlDoc->createElement('title', $item->from_person_signatory_title));
                }
                if($item->from_person_signatory_first_name !== null && $item->from_person_signatory_first_name !== '')
                {
                    $t_person->appendChild($xmlDoc->createElement('first_name', $item->from_person_signatory_first_name));
                }
                if($item->from_person_signatory_last_name !== null && $item->from_person_signatory_last_name !== '')
                {
                    $t_person->appendChild($xmlDoc->createElement('last_name', $item->from_person_signatory_last_name));
                }
                if($item->from_person_signatory_birthdate !== null && $item->from_person_signatory_birthdate !== '')
                {
                    $dateofbirth = new DateTime($item->from_person_signatory_birthdate);
                    $formattedDateofbirth = $dateofbirth->format('Y-m-d\TH:i:s');
                    $t_person->appendChild($xmlDoc->createElement('birthdate', $formattedDateofbirth));
                }
                if($item->from_person_signatory_ssn !== null && $item->from_person_signatory_ssn !== '')
                {
                    $t_person->appendChild($xmlDoc->createElement('ssn', $item->from_person_signatory_ssn));
                }
                if($item->from_person_signatory_nationality1 !== null && $item->from_person_signatory_nationality1 !== '')
                {
                    $t_person->appendChild($xmlDoc->createElement('nationality1', $item->from_person_signatory_nationality1));
                }
                if($item->from_person_signatory_residence !== null && $item->from_person_signatory_residence !== '')
                {
                    $t_person->appendChild($xmlDoc->createElement('residence', $item->from_person_signatory_residence));
                }
                $addresses = $xmlDoc->createElement('addresses');
                    $address = $xmlDoc->createElement('address');
                        if($item->from_person_signatory_address_type !== null && $item->from_person_signatory_address_type !== '')
                        {
                            $address->appendChild($xmlDoc->createElement('address_type', $item->from_person_signatory_address_type));
                        }
                        if($item->from_person_signatory_address !== null && $item->from_person_signatory_address !== '')
                        {
                            $address->appendChild($xmlDoc->createElement('address', $item->from_person_signatory_address));
                        }
                        if($item->from_person_signatory_city !== null && $item->from_person_signatory_city !== '')
                        {
                            $address->appendChild($xmlDoc->createElement('city', $item->from_person_signatory_city));
                        }
                        if($item->from_person_signatory_country_code !== null && $item->from_person_signatory_country_code !== '')
                        {
                            $address->appendChild($xmlDoc->createElement('country_code', $item->from_person_signatory_country_code));
                        }
                    $addresses->appendChild($address);
                $t_person->appendChild($addresses);

                if($item->from_person_signatory_occupation !== null && $item->from_person_signatory_occupation !== '')
                {
                    $t_person->appendChild($xmlDoc->createElement('occupation', $item->from_person_signatory_occupation));
                }

                // Append the <from_person> element to the <t_from_my_client> element
                $signatory->appendChild($t_person);

                if($item->from_person_signatory_role !== null && $item->from_person_signatory_role !== '')
                {
                    $signatory->appendChild($xmlDoc->createElement('role', $item->from_person_signatory_role));
                }
        ...
        // dd($scenario_type);
        // \DB::enableQueryLog();
        $indicators = ScenarioThree::select('report_indicator')
            ->whereBetween('date_transaction', [$from_date, $to_date])
            ->where('xml_gen_status', '=', 'N')
            ->where('is_delete', '=', 0)
            ->where('status', '=', 'Y')
        ...
        $fileName = 'files/xmlfile_' . time() . '_scenario_three.xml';
        $xmlDoc->save(storage_path('app/public/' . $fileName));
        $scenario_no = 3;

        // Log the creation of the XML
        \Log::info('User ' . $user->id . ' created an XML file for Scenario 3 at ' . Carbon::now());
        \LogActivity::addToLogXMLGen('User ' . $user->id . ' created an XML file for Scenario 3 at ' . Carbon::now(),$from_date,$to_date,$fileName,$xml_type,$scenario_no,$xml_gen_status);
    ...
    public function downloadExcel($id)
    {
        ...
        $excel_data = ScenarioThree::select('*')
            ->where('is_delete', '0')
        ...
        $filename = Carbon::now().'-scenario-three.csv';
        ...
        $columns = [
            'scenario_type', 'rentity_id', 'rentity_branch', 'submission_code', 'report_code',
            'entity_reference', 'submission_date', 'currency_code_local', 'transaction_location', 'transactionnumber',
            ...
            'from_account_non_bank_institution', 'from_account_branch', 'from_account_account', 'from_account_currency_code',
            'from_account_personal_account_type', 'from_account_name', 'from_account_incorporation_legal_form',
            'from_account_incorporation_number', 'from_account_business', 'from_account_address_type', 'from_account_address',
            'from_account_city', 'from_account_country_code', 'from_account_incorporation_country_code',
            'from_account_director_gender', 'from_account_director_title', 'from_account_director_first_name',
            'from_account_director_last_name', 'from_account_director_birthdate', 'from_account_director_ssn',
            'from_account_director_passport_number', 'from_account_director_passport_country',
            'from_account_director_nationality1', 'from_account_director_residence', 'from_account_director_address_type',
            'from_account_director_address', 'from_account_director_city', 'from_account_director_country_code',
            'from_account_director_occupation', 'from_account_director_role', 'status_code', 'from_country',
            'to_funds_code', 'to_account_institution_name', 'to_account_swift', 'to_account_non_bank_institution',
            'to_account_account', 'to_account_currency_code', 'to_country', 'report_indicator',
            'from_person_signatory_is_primary', 'from_person_signatory_gender', 'from_person_signatory_title',
            'from_person_signatory_first_name', 'from_person_signatory_last_name', 'from_person_signatory_birthdate',
            'from_person_signatory_ssn', 'from_person_signatory_nationality1', 'from_person_signatory_residence',
            'from_person_signatory_address_type', 'from_person_signatory_address', 'from_person_signatory_city',
            'from_person_signatory_country_code', 'from_person_signatory_occupation', 'from_person_signatory_role'
        ];
        ...
        $callback = function() use ($excel_data, $columns) {
            ...
            foreach ($excel_data as $row) {
                fputcsv($file, [
                    ...
                    $row->from_account_swift, $row->from_account_non_bank_institution, $row->from_account_branch, $row->from_account_account,
                    $row->from_account_currency_code, $row->from_account_personal_account_type, $row->from_account_name, $row->from_account_incorporation_legal_form,
                    $row->from_account_incorporation_number, $row->from_account_business, $row->from_account_address_type, $row->from_account_address,
                    $row->from_account_city, $row->from_account_country_code, $row->from_account_incorporation_country_code, $row->from_account_director_gender,
                    ...
                    $row->report_indicator, $row->from_person_signatory_is_primary, $row->from_person_signatory_gender, $row->from_person_signatory_title,
                    $row->from_person_signatory_first_name, $row->from_person_signatory_last_name, $row->from_person_signatory_birthdate,
                    $row->from_person_signatory_ssn, $row->from_person_signatory_nationality1, $row->from_person_signatory_residence,
                    $row->from_person_signatory_address_type, $row->from_person_signatory_address, $row->from_person_signatory_city,
                    $row->from_person_signatory_country_code, $row->from_person_signatory_occupation, $row->from_person_signatory_role
                ]);
            }
