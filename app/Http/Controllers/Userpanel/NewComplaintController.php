...
class NewComplaintController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(!empty($request->complain_category_id)){
            ...
            $cityInfo = LabourOfficeCityDetail::select('labour_office_city_details.office_code','labour_office_city_details.office_id')
                    ->where('labour_office_city_details.city_id', $request->city_id)
                    ->join('labour_offices_divisions','labour_offices_divisions.id', '=', 'labour_office_city_details.office_id')
                    ->where('labour_offices_divisions.office_type_id', 4)
                    ->first();
            ...
            if (!empty($cityInfo)) {
                ...
                $cityInfo = LabourOfficeCityDetail::select('labour_office_city_details.office_code','labour_office_city_details.office_id')
                    ->join('labour_offices_divisions','labour_offices_divisions.id', '=', 'labour_office_city_details.office_id')
                    ->where('labour_office_city_details.city_id', $request->city_id)
                    ->where('labour_offices_divisions.office_type_id', 5)
                    ->first();
                if (!empty($cityInfo)) {
                    $office_code = $cityInfo->office_code;
                    $office_id = $cityInfo->office_id;
            ...
            if($request->complainant_mobile != ''){
                ...
                if($regdata[0]->pref_lang == 'EN'){
                    ...
                    $variableData = [$request->current_employer_name,$request->current_employer_name,$request->employer_name,$new_complaint_no,$request->current_employer_address,$officename->office_name_en,$request->complainant_f_name.' '.$request->complainant_l_name,$request->complainant_address];
                    ...
                    $variableData = [$request->current_employer_name,$request->employer_address_ta,$request->employer_name_ta,$new_complaint_no,$request->current_employer_address,$officename->office_name_tam,$request->complainant_f_name.' '.$request->complainant_l_name,$request->complainant_address];
