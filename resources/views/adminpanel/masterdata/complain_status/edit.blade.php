            ...
            <div class="col-lg-12">
                    <div class="row cms_top_btn_row" style="margin-left:auto;margin-right:auto;"> 
                        <a href="{{ route('complain-status') }}">
                            <button class="btn cms_top_btn top_btn_height ">{{ __('complainstatus.add_new') }}</button>
                        </a>
            ...
            <div class="jarviswidget" id="wid-id-1" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-custombutton="false" role="widget">
                ...
                <div>
                    ...
                    <div class="widget-body no-padding">
                        <form action="{{ route('save-complain-status') }}" enctype="multipart/form-data" method="post" id="complain-status-form" class="smart-form">
                        @csrf
                        @method('PUT')
                            <fieldset>
                                <div class="row">
                                        ...
                                        <label class="select">
                                            <select name="complaint_status_type_id" id="complaint_status_type_id" required>
                                                ...
                                                @foreach ($statustypes as $statustype)
                                                    <option value="{{ $statustype->id }}" {{$data->complaint_status_type_id == $statustype->id ? 'selected' : ''}}>@if($lang == "SI"){{ $statustype->type_name_si }}@elseif($lang == "TA"){{ $statustype->type_name_ta }}@else{{ $statustype->type_name_en }}@endif</option>
                                                @endforeach
                                            </select>
                                            <i></i>
                                        </label>
                                    ...
                                    <section class="col col-4">
                                        ...
                                        <label class="input">
                                            <input type="text" id="status_en" name="status_en" required value="{{ $data->status_en }}">
                                        </label>
                                    </section>
                                    <section class="col col-4">
                                        <label class="label">{{ __('complainstatus.complain_status_sin') }}<span style=" color: red;">*</span> </label>
                                        <label class="input">
                                            <input type="text" id="status_si" name="status_si" required value="{{ $data->status_si }}">
                                        </label>
                                    </section>
                                </div>
                                <div class="row">
                                    <section class="col col-4">
                                        ...
                                        <label class="input">
                                            <input type="text" id="status_ta" name="status_ta" required value="{{ $data->status_ta }}">
                                        </label>
                                    </section>
                                    <section class="col col-4">
                                        ...
                                        <label class="select">
                                            <select name="status" id="status">
                                                <option value="Y" {{ $data->status == 'Y' ? "selected" : "" }}>{{ __('complainstatus.status_active') }}</option>
                                                <option value="N" {{ $data->status == 'N' ? "selected" : ""  }}>{{ __('complainstatus.status_inactive') }}</option>
                                            </select>
                                            <i></i>
                                        </label>
                                    </section>
